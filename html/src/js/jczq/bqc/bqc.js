define([
    "text!jczq/bqc/bqc_list_item.tpl",
    "jczq/up_sp",
    "jczq/topView",
    "lib/tools",
	"login/art_login",
	...
	"lib/json2"
],function(bqc_list_item,updateSP,topView,tools,art_login,dialogAlert,pay,stop_game,C,ls,jstorage){
	if ($("#jczqbqc").length == 0) return function () {
    };
    // 更新页面数据
    //updatePage();
   return function(){
	   jstorage();
	    ...
	    var _nickNameHash = {"11533": "胜胜", "11531": "胜平", "11530": "胜负", "11513": "平胜", "11511": "平平", "11510": "平负", "11503": "负胜", "11501": "负平", "11500": "负负"};
	 ...
	 /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
	    ...
	    var listItemView = Backbone.View.extend({
	        ...
	        render: function () {
	            ...
	            this.$el.addClass(className).attr("index", this.model.get("index")).html(_nickNameHash[code]);
	            return this
	        }
	    });
	    var AppView = Backbone.View.extend({
	        ...
	        listTemplate: _.template(bqc_list_item),
	        plus: function () {
	            var $this = this.$mul;
	            var v = $this.val();
	            if (v < 9999) {
	        ...
	        calc:function(){
	            ...
	            if( isVisibile && $inputs.find("input[passtype]:checked").length>0 && mul>0){
	            	...
	            	_.each(count[1],function(matchid,k){
	            		...
	            		matches[k] = matchsp;
	            		if(me.$("input[betsmatchid="+matchid+"]").prop("checked")){
	            			dans.push(k);
	            		}
	            	});
	            	...
	            	var count = C.JCZQ_Calc.calCount(true, matches, dans, types, [0,0,0,0,0,0,0,0,0]);
	            	award = C.JCZQ_Calc.calExtreme(true, matches, dans, types, [0,0,0,0,0,0,0,0,0]);
	           ...
	           isLogin(function(userId) {
	               if (userId.indexOf("qq.sohu.com") > -1 && $.jStorage.index().length > 0) {
	                   ...
	                   if (Math.abs( + new Date - ( + dataObj.timestamp)) > 60 * 1000) {
	                       $.jStorage.flush();
	                   } else if (dataObj && dataObj.gameId === "bqc") {
